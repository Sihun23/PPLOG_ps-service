...
public class PsController {
    ...
    @Operation(summary = "API 명세서 v0.4 line 67", description = "자기소개서 목록 조회")
    @GetMapping
    ...
    @Operation(summary = "API 명세서 v0.4 line 68", description = "자기소개서 조회")
    @GetMapping("/{psId}")
    ...
    @Operation(summary = "API 명세서 v0.4 line 69", description = "자기소개서 작성")
    @PostMapping
    public ResponseEntity<ApiResponse<PsIdResponse>> createPs(
            @RequestHeader("memberId") String memberId,
            @Valid @RequestBody PsRequest psRequest) {
        Long convertedMemberId = Long.valueOf(memberId);
        PsIdResponse response = psService.createPs(psRequest, convertedMemberId);
        return ResponseEntity.ok(ApiResponse.onSuccess(response));
    }

    @Operation(summary = "API 명세서 v0.4 line 70", description = "자기소개서 수정")
    @PutMapping("/{psId}")
    ...
    @Operation(summary = "API 명세서 v0.4 line 71", description = "자기소개서 목록 삭제")
    @DeleteMapping("/{psId}")
    ...
    @Operation(summary = "API 명세서 v0.4 line 72", description = "자기소개서 목록 첨삭")
    @GetMapping("/{psId}/editing")
    public ResponseEntity<ApiResponse<PsResponse>> editPs(
            @RequestHeader("memberId") String memberId, // memberId를 String으로 받음
            @PathVariable Long psId) {
        Long convertedMemberId = Long.valueOf(memberId);
